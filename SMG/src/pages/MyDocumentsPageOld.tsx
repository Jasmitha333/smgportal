        ...
        <div className="space-y-4">
          <div>
            ...
            <label
              ...
              className="block border-2 border-dashed border-gray-200 rounded-xl p-8 text-center hover:border-[#0B4DA2]/50 transition-colors cursor-pointer"
            >
              <Upload size={32} className="mx-auto mb-2 text-gray-400" />
              <p className="text-sm font-bold text-[#1B254B] mb-1">
...
export const MyDocumentsPageOld = ({ documents, user }) => {
  ...
  const [generalDocTitle, setGeneralDocTitle] = useState('');
  const [generalDocCategory, setGeneralDocCategory] = useState('Select Category');
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-6 border border-[#0B4DA2]/20 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className="bg-[#0B4DA2] text-white p-2.5 rounded-lg">
              <Upload size={20} />
            </div>
            <div>
              <h3 className="font-bold text-[#1B254B] text-base">Upload Document</h3>
              <p className="text-xs text-[#A3AED0]">Add documents to your records</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <label className="block text-xs font-bold text-[#1B254B] mb-2">Document Title</label>
            <input
              type="text"
              placeholder="Enter name"
              value={generalDocTitle}
              onChange={(e) => setGeneralDocTitle(e.target.value)}
              className="w-full px-3 py-2.5 bg-white rounded-lg border border-gray-200 focus:border-[#0B4DA2] outline-none transition-colors text-sm"
            />
          </div>

          <div>
            <label className="block text-xs font-bold text-[#1B254B] mb-2">Category</label>
            <select
              value={generalDocCategory}
              onChange={(e) => setGeneralDocCategory(e.target.value)}
              className="w-full px-3 py-2.5 bg-white rounded-lg border border-gray-200 focus:border-[#0B4DA2] outline-none transition-colors text-sm"
            >
          ...
          <div>
            <label className="block text-xs font-bold text-[#1B254B] mb-2">Upload File</label>
            <button
              onClick={() => setIsUploadModalOpen(true)}
              className="flex items-center justify-center gap-2 w-full px-3 py-2.5 bg-white border border-dashed border-[#0B4DA2] rounded-lg cursor-pointer hover:bg-blue-50 transition-colors"
            >
              <Upload size={16} className="text-[#0B4DA2]" />
              <span className="text-sm font-medium text-[#0B4DA2]">Choose File</span>
            </button>
          </div>
        </div>

        <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-200">
          <p className="text-xs text-[#A3AED0]">Supported: PDF, DOC, DOCX, JPG, PNG (Max 10MB)</p>
          <button
            onClick={() => {
              if (generalDocTitle && generalDocCategory !== 'Select Category') {
                setIsUploadModalOpen(true);
              } else {
                alert('Please fill in document title and category first');
              }
            }}
            className="bg-gradient-to-r from-[#0B4DA2] to-[#042A5B] text-white px-6 py-2 rounded-lg font-bold text-sm hover:shadow-lg transition-all flex items-center gap-2 active:scale-95"
          >
            <Upload size={16} />
            Upload
          </button>
